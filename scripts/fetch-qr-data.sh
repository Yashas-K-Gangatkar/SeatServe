#!/usr/bin/env bash
# Task 25-b — pull live seat-QR data for every classroom (asha session).
# Output: /tmp/nf-qr-all.json = [{ classroom, block, seats: [{code, qrToken}] }]
set -uo pipefail
...
print('classrooms:', [(s['name'], s['seatsCount']) for s in d['classrooms']])
EOF
...
d = json.load(open('/tmp/nfq.json'))['data']
all_screens = []
for s in d['classrooms']:
    sid = s['id']
    out = subprocess.run(['curl', '-s', '-b', '/tmp/nf-asha3.jar',
                          f"https://notifetch.in/api/admin/qr?classroomId={sid}"],
                         capture_output=True, text=True).stdout
    sd = json.loads(out)['data']
    all_screens.append({
        'classroom': sd['classroom']['name'],
        'block': sd['classroom']['block'],
        'seats': [{'code': x['code'], 'qrToken': x['qrToken']} for x in sd['seats']],
    })
    print(f"{sd['classroom']['name']}: {len(sd['seats'])} seats")
