...
// the batch to PROCESSED and its snapshot rows to SETTLED. CAMPUS_ADMIN only.
...
export async function POST(request: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  // the batch's store must belong to the caller's campus
  ...
  if (settlement.store.campusId !== user.campusId) return fail('This batch belongs to another campus', 403)
