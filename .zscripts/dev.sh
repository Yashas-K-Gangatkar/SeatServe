...
echo "[BUN] Starting server..."
# Prefer the production standalone build (stable, ~7x less memory) when it
# exists and scripts/.prod-mode is set; otherwise fall back to dev mode.
if [ -f .next/standalone/server.js ] && [ -f scripts/.prod-mode ]; then
        echo "[BUN] Production mode (standalone)..."
        bun run start &
else
        echo "[BUN] Development mode..."
        bun run dev &
fi
DEV_PID=$!
...
# Server watchdog — sibling of the server process so it gets the same
# lifetime; restarts :3000 automatically if it stops responding.
pkill -f "scripts/dev-watchdog.sh" 2>/dev/null || true
rm -f /tmp/seatserve-watchdog.lock
nohup bash "$PROJECT_DIR/scripts/dev-watchdog.sh" >/dev/null 2>&1 &
WATCHDOG_PID=$!
disown "$WATCHDOG_PID" 2>/dev/null || true
echo "Watchdog started in background (PID: $WATCHDOG_PID)."
